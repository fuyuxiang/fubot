...
try:
    ...
    raise ImportError(
        "Matrix dependencies not installed. Run: pip install fubot-ai[matrix]"
    ) from e

from fubot.bus.events import OutboundMessage
from fubot.bus.queue import MessageBus
from fubot.channels.base import BaseChannel
from fubot.config.paths import get_data_dir, get_media_dir
from fubot.utils.helpers import safe_filename
...
class MatrixChannel(BaseChannel):
        ...
        if self._sync_task:
            try:
                ...
                try:
                    ...
                    logger.debug("Matrix sync task cancelled during shutdown")
        if self.client:
    ...
    async def _set_typing(self, room_id: str, typing: bool) -> None:
        ...
        try:
            ...
            logger.debug("Matrix typing update failed for {}", room_id)
    ...
    async def _stop_typing_keepalive(self, room_id: str, *, clear_typing: bool) -> None:
        if task := self._typing_tasks.pop(room_id, None):
            ...
            try:
                ...
                logger.debug("Matrix typing keepalive cancelled for {}", room_id)
        if clear_typing:
