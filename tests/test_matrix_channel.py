...
import fubot.channels.matrix as matrix_module
from fubot.bus.events import OutboundMessage
from fubot.bus.queue import MessageBus
from fubot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from fubot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("fubot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "fubot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("fubot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "fubot.channels.matrix.asyncio.create_task", _fake_create_task
    )
